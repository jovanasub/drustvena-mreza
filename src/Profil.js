import React from "react";
import "./Profil.css"

function Profil({ profil, izmenaProfile }) {
  return (
    <div className="profilStrana">
      <img width="100%" height={250} src="data:image/jpeg;base64," />
      <img src={profil.slika} />
      <div>Ovde mozete izmeniti informacije profila</div>
      <input value={profil.ime} onChange={(e) => izmenaProfile("ime", e.target.value)} />
      <input value={profil.email} onChange={(e) => izmenaProfile("email", e.target.value)} />
      <input value={profil.datumRodjenja} onChange={(e) => izmenaProfile("datumRodjenja", e.target.value)} />
    </div>
  );
}

export default Profil;
